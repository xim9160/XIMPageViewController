...
#import "ChannelEditController.h"
...
#define ButtonHeight floor((ButtonWidth * 4/9))
#define LocationWidth (ScreenWidth - EdgeX * 2)
#define ButtonWidth floor((LocationWidth/ButtonCountOneRow))
...
#define Header_TitLe_font 12
#define Header_Sub_font 9
#define Edit_Button_Height 44

@interface ChannelEditController ()
{
    BOOL _isEditing;
    CGPoint _oldCenter;
...
@implementation ChannelEditController

//todo xiaofengmin rewrite
...
- (UILabel *)topLabel {
    if (!_topLabel) {
        ...
        _topLabel.frame = CGRectMake(16, Header_Height - Header_TitLe_font , 50, Header_TitLe_font);
        if(!_topLabel.superview) {
...
- (UIScrollView *)scrollView {
    if (!_scrollView) {
        ...
        _scrollView.frame = CGRectMake(0, Header_Height + 16, ScreenWidth, self.view.frame.size.height - Header_Height);
...
- (UIButton *)editButton {
    if (!_editButton) {
        ...
        _editButton.frame = CGRectMake(ScreenWidth - 16 - 88, Header_Height - Header_TitLe_font , 88, Header_TitLe_font);
//        [_editButton setFont:[UIFont systemFontOfSize:Header_TitLe_font]];
        [_editButton.titleLabel setFont:[UIFont systemFontOfSize:Header_TitLe_font]];
...
- (UILabel *)editAlertLabel {
    if (!_editAlertLabel) {
        _editAlertLabel = [[UILabel alloc] init];
        _editAlertLabel.frame = CGRectMake(16 + 50 + 16, Header_Height - Header_Sub_font , 200, Header_Sub_font);
        _editAlertLabel.text = @"长按或者点击编辑";
        if(!_editAlertLabel.superview) {
            [self.view addSubview:_editAlertLabel];
        }
    }
    return _editAlertLabel;
}
...
- (void)viewWillDisappear:(BOOL)animated {
    if (_disAppearBlock != nil) {
        _disAppearBlock(self.topDataSource, self.bottomDataSource);
    }
}
...
-(void)bottomTapAct:(UITapGestureRecognizer *)tap{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        self.bottomLabel.frame = CGRectMake(10, TopEdge + 25 + self.topHeight, 200, 20);
        [self reconfigBottomView];
        touchView.closeImageView.hidden = !self->_isEditing;
    }];
...
- (void)topPanAct:(UIPanGestureRecognizer *)pan{
    ...
    if (pan.state == UIGestureRecognizerStateBegan) {
        ...
        CGFloat x = touchView.center.x;
        CGFloat y = touchView.center.y;
        if (x < EdgeX || x > ScreenWidth - EdgeX || y < TopEdge || (y < (TopEdge + ButtonHeight) && x < (EdgeX + 2 * ButtonWidth))) {
            NSLog(@"超出范围");
            [UIView animateWithDuration:0.5 animations:^{
                touchView.center = self->_oldCenter;
            }];
        } else{
